...
def solveScrambleAllDRAxis(scramble,fromEOToDRPrune,fromDRPrune):
    ...
    for scramble,rev in zip(scrambleList,reversing):
        ...
        resDic["EO:"] = (" ".join(eoSolPrint),len(eoSol))
        for move in eoSol:
            startc,starte = mdic[move](startc,starte)
        ...
        for move in drSol:
            startc,starte = mdic[move](startc,starte)
        resDic["DR:"] = (" ".join(drSolPrint),len(drSolPrint))
        
        finalSol = solveFromDRFast(startc,starte,given=fromDRPrune)
        finalSolPrint = doReverseRotate(finalSol,rev)

        resDic["Finish:"] = (" ".join(finalSolPrint),len(finalSolPrint))

        resDic["Solution length:"] = (len(eoSol)+len(drSol)+len(finalSol),'')

        solution_texts.append(resDic)
    return solution_texts


def solveScrambleAllDRAxisExtraDRStep(scramble,fromEOToDRPrune,fromDRPrune):
    print("Scramble:",scramble)
    scramble = scramble.split(' ')
    scrambleList,reversing = rotatedScrambles(scramble)
    ...
    for scramble,rev in zip(scrambleList,reversing):
        ...
        for move in human1drSol:
            startc,starte = mdic[move](startc,starte)
        resDic["DR substep:"] = (" ".join(human1drSolPrint),len(human1drSolPrint))
        

        drSol = EOToDR(startc,starte,scramble,eoSol,partialStep=human1drSol,given=fromEOToDRPrune)
        drSolPrint = doReverseRotate(drSol,rev)
